...
unsigned long romfs_drive;              // new for romfs support.
#ifndef STAGE1_5
...
char *err_list[] =
{
  ...
  [ERR_BAD_ROMFS_IMAGEFILE] = "Romfs Image file invalid or corrupt",
...
// from Grub4DOS 0.4.1
  ...
  if (cdrom_drive == GRUB_INVALID_DRIVE)  
      for (drive = 0x88; drive <= 0xff; drive++)
      {
        struct geometry geom;
  ...
  // ------------------------------- set romfs drive 
      /* Get the geometry.  */
    if (romfs_drive != ROMFS_DRIVE)
      romfs_drive = GRUB_INVALID_DRIVE;
    else
      romfs_drive = boot_drive;     // here boot_drive == ROMFS_DRIVE

	/* we move cmain from common.c to here, for install internal 
	 *   romfs. ;-)
	 */
    grub_memmove(ROMFS_LOAD_ADDR, module_romfs_gz, end_of_module_romfs_gz - module_romfs_gz);
