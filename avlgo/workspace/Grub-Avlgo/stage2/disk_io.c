  ...
  if (*device == '(' && *(++device))
      ...
      if (*device != ',' && *device != ')')
	{
	  ...
	  if (*device == 'f' || *device == 'h'
          ...
          || (*device == 'r' && romfs_drive != GRUB_INVALID_DRIVE)) // new for romfs
#else
	  if (*device == 'f' || *device == 'h'
	      || (*device == 'c' && cdrom_drive != GRUB_INVALID_DRIVE)
          || (*device == 'r' && romfs_drive != GRUB_INVALID_DRIVE)) // new for romfs
...
#endif
	       || (*device == 'c' && cdrom_drive != GRUB_INVALID_DRIVE)
           || (*device == 'r' && romfs_drive != GRUB_INVALID_DRIVE))
	      && (device += 2, (*(device - 1) != 'd')))
	  ...
	  if (ch == 'n' && network_ready)
...
#endif /* SUPPORT_NETBOOT */
	    {
	      if (ch == 'c' && cdrom_drive != GRUB_INVALID_DRIVE)
		    current_drive = cdrom_drive;
          else if(ch == 'r' && romfs_drive != GRUB_INVALID_DRIVE)
		    current_drive = ROMFS_DRIVE;  //romfs_drive;
	      else
		{
    ...
    if (*buf == '/' || (ptr = set_device (buf)) || incomplete)
    {
        ...
        if (*buf == '(' && (incomplete || ! *ptr))
        {
            if (! part_choice)
            {
                ...
                // new for romfs
                if (romfs_drive != GRUB_INVALID_DRIVE
                && (disk_choice || romfs_drive == current_drive)
                && (!ptr
                  || *(ptr-1) == '('
                  || (*(ptr-1) == 'd' && *(ptr-2) == 'r')))
  ...
  if (*filename != '/')
      ...
      if (list_addr < BLK_MAX_ADDR && ptr != filename && !errnum)
	{
	  ...
	  //return gunzip_test_header ();
        nRet = gunzip_test_header ();

        if(!compressed_file)            // no gzip?
            nRet = pkunzip_test_header();
