...
print_root_device (void)
{
  ...
  else if (saved_drive == ROMFS_DRIVE)
    {
      /* Network drive.  */
      grub_printf (" (rd):");
    }
...
//////////////////////////////////////////////////////////////////////////
// new for romfs                              (r) powered by Gandalf
//    specify a romfs image file, and emulate a disk in memory
//
//////////////////////////////////////////////////////////////////////////
#define ROMFS_MAGIC         "-rom1fs-"
static int romfs_func(char *arg, int flags) 
{
    char chHead[8];
    ...
    // OK, read this image file to memory and checks for it.
	
    // check for head
    grub_read (chHead, 8); 
    if(grub_memcmp(chHead, ROMFS_MAGIC, 8) !=0)
    {
        printf("\n not a valid romfs image file.\n");
        return false;
    }
    grub_close();
	...
	grub_read ((char *) ROMFS_LOAD_ADDR, -1); 
	
    romfs_drive = ROMFS_DRIVE;              // Enable the drive
    
    grub_printf("\n romfs dirve is now ready for use. \n");
...
static struct builtin builtin_romfs =
{
  "romfs",
  romfs_func,
  BUILTIN_CMDLINE | BUILTIN_MENU | BUILTIN_HELP_LIST,
...
//////////////////////////////////////////////////////////////////////////
// new for isoemu                              (r) powered by Gandalf
//    specify a iso image file, emulate a disk and boot from it
//
//////////////////////////////////////////////////////////////////////////
static int isoemu_func(char *arg, int flags) 
{
    int n, i;
    
    unsigned char *pISOEmu = (unsigned char *)RAW_ADDR(0x50000);
    ...
    // if(if (saved_drive & 0x80)
    if(current_drive > 0x88 || current_drive < 0x80)
    {
        grub_printf("\n This verion only supports hdd. \n");
        return false;
    }
    ...
    for(i=0; i < grub_strlen(arg); i ++)
        if(arg[i] == '/')
            break;
    
    if(i != n)
    {
        printf("\n Only support iso file in root-dir.");
        return false;
    }

    //
    // OK, read this image file to memory 0x0000:0x800 and patch it.
    //
    
    // open our isoemu.gz
    grub_close();

    if(!grub_open("(rd)/isoemu.gz"))
	{   // open failed
        grub_close();
        grub_printf("\n open file failed(internal error). \n");
        return false;
    }

    // now copy the (rd)/isoemu to 0x0:0x800
    grub_read(pISOEmu, -1);

    // patch it with the given name
    grub_toupper(&arg[n+1]);
    grub_memmove(&pISOEmu[4], &arg[n+1], 12);

    // grub_memmove((char *)ISOEMU_LOCATION, pISOEmu, 0x8000-0x800);
    // grub_memmove((char *)ISOEMU_LOCATION, pISOEmu, 0x200);
...
static struct builtin builtin_isoemu =
{
  "isoemu",
  isoemu_func,
  BUILTIN_CMDLINE | BUILTIN_MENU | BUILTIN_HELP_LIST,
  "isoemu [FILE]",
  "Specify a non-emu mods' iso file(located in root directory of a" 
  " FAT32/NTFS partition) to emulate a CD-ROM and boot boot from it."
};


//////////////////////////////////////////////////////////////////////////
// new for version                              (r) powered by Gandalf
//    Display the version of Avlgo (based on Grub 0.97)
//
//////////////////////////////////////////////////////////////////////////
static char version_info[] = "\n" AVLGO_LONGNAME "\n" \
" (R)Powered by " AVLGO_AUTHOR "(" AVLGO_EMAIL "), " AVLGO_YEARS "\n" \
" (W)Web Site  : " AVLGO_HOMEPAGE "\n" \
" (L)Lisence   : " AVLGO_LICENCE "\n" \
" (B)Build Date: " AVLGO_COMPILIEDTIME "\n";

static int version_func(char *arg, int flags) 
{
    grub_printf(version_info);
    return true;
}
...
static struct builtin builtin_version =
{
  ...
  BUILTIN_CMDLINE | BUILTIN_HELP_LIST,
  "version",
  "Display the version of Avlgo(based on Grub, mini-set)." 
};
