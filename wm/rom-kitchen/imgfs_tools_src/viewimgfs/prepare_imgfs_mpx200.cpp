...
void main(int argc, char **argv)
{
	if(argc<2)
	{
		puts("Usage: prepare_imgfs.exe nk.nba [-nosplit]");
		return;
	}
	...
	DWORD Tmp;
	printf("Dumping... ");
    for(int i=0; i<Size; i+=0x20000)
    {
    	WriteFile(H,ImgfsPtr+i,0x1F800,&Tmp,0);
    	WriteFile(HT,ImgfsPtr+i+0x1F800,0x800,&Tmp,0);
    	FlushFileBuffers(H);
    }
	puts("Done!");
}
